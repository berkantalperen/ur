...
class SafetyNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("Safety Node Started (Synchronized Time Offset)")
    ...
    def raw_callback(self, msg_raw):
        ...
        msg_safe.header = msg_raw.header 
        msg_safe.header.frame_id = self.robot_base_frame 
    ...
    def control_loop(self):
        ...
        # --- TIME SYNC FIX ---
        # Apply the same 50ms offset so this command matches the Robot State time
        now_ns = self.get_clock().now().nanoseconds
        msg.header.stamp = self.get_clock().now().to_msg() # RESTORE THIS
        # ---------------------
    ...
    def publish_visualization(self, safe_pos):
        ...
        marker_pt.color.b = 1.0; marker_pt.color.a = 1.0 
